...
<div class="container-fluid">
    ...
    <div class="row clearfix">
        <form action="{{ route('author.post.update', $post->id) }} "
            method="POST"
            ...
            <div class="col-lg-7 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        <button type="submit"
                            class="btn btn-primary m-t-10 waves-effect">Update</button>
                    </div>
                </div>
            </div>
                    ...
                    <div class="body">
                        <div class="form-group">
                            ...
                            <select multiple
                                ...
                                <option @foreach($post->categories as $pc)
                                    {{ $pc->id == $category->id ? 'selected': '' }}
                                    @endforeach
                                    value="{{ $category->id }}">
                        ...
                        <div class="form-group">
                            ...
                            <select multiple
                                ...
                                required>
                                @foreach ($tags as $tag)
                                <option @foreach($post->tags as $pt)
                                    {{ $pt->id == $tag->id ? 'selected': '' }}
                                    @endforeach
                                    value="{{ $tag->id }}">{{ Str::ucfirst($tag->name)  }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
